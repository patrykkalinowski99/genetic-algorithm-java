...
//            tab_laczona[0].replace(" ", "").length()
        ...
        System.out.println("\n\nKrzyżowanie 4-punktowe");
        System.out.println("Łączenie w pary:");
        int numer_indeksu = 1;
        for (int i = 0; i<rozmiar_par/2; i++){
            int ilosc_punktow = 3;//zmienic na m
            if (ilosc_punktow == 0) ilosc_punktow++;
            int[] losowe_punkty = new int[ilosc_punktow];
            losowe_punkty[0] = r.nextInt(zakres_m) + 1;
            if (losowe_punkty[0]< 8){
                losowe_punkty[0] += 4;
            }
            losowe_punkty[1] = r.nextInt(losowe_punkty[0]) + 1;
            if (losowe_punkty[1] == losowe_punkty[0]){
                    losowe_punkty[1] -= 1;
            }
            if (losowe_punkty[1] < 5){
                losowe_punkty[1] += 5;
            }
            losowe_punkty[2] = r.nextInt(losowe_punkty[1]);
            if (losowe_punkty[2] == losowe_punkty[1]){
                losowe_punkty[2] -= 1;
            }
            if (losowe_punkty[2] == losowe_punkty[1]){
                losowe_punkty[2] -= 1;
            ...
            for (int k = ilosc_funkcji - 1; k >= 0; k--) {
                ...
                while (true) {
                    if (zakresik_m == zakres_m_po_odliczeniu || zakresik_m == -1)
                        ...
                        dziesietna += populacja_array_klon[i][zakresik_m] * Math.pow(2, potega);
                        potega++;
...
//                krzyzowanie_array_min_fx[i] = ocena_potomkow[i];
        ...
        for (int i = 0; i < n; i++){
            System.out.print((i+1)+".\t");
            for (int j = 0; j< zakres_m; j++){
                System.out.print(populacja_array_klon[i][j]+" ");
            }
        ...
        suma /= n;
        System.out.println("Suma Fx = "+suma);
        //////////////////////////////////////ROWNOMIERNE//////////////////////////////////////
