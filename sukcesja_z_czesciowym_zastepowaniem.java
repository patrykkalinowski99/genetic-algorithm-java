...
public class sukcesja_z_czesciowym_zastepowaniem {
    public static void main(String[] args) {
        ...
        //tablica sukcesji
        String[] sukcesja = new String[ilosc_funkcji*n];
        int iterator_sukcesji = 0;
        for (int i = 0; i<sukcesja.length; i++){
            sukcesja[i] = "";
        }
        for (int i = 0; i<n; i++){
            sukcesja[i] = tab_laczona[i].replace(" ", "");
            iterator_sukcesji++;
        }
        //////////////////
...
//        }
        Random r = new Random();
        System.out.println();////////////////////////////////////////////////////////////////////////////////////////////////////////////
        System.out.println("Algorytm genetyczny - SUKCESJA Z CZĘSCIOWYM ZASTĘPOWANIEM");
        //for (int epoka = 1; epoka <= 10; epoka++) {
            ...
            for(int i = 0; i < n; i++){
                System.out.println((i+1)+".\t"+tab_laczona[i].replace("", "")+"\tFx = "+Fx[i]);
            }
            ...
            for (int i = 0; i<n; i++){
                ...
                for (int k = 0; k<zakres_m; k++){
                    sukcesja[iterator_sukcesji] += populacja_array[i][k];
                }
                iterator_sukcesji++;
            }
            double value = 0;
            ...
            for (int i = 0; i<n; i++){
                ...
                if(r_chromosom[i]<prawdopodobienstwo_inwersji){
                    ...
                    for (int j = 0; j<zakres_m; j++) {
                        sukcesja[iterator_sukcesji] += populacja_array[i][j];
                    }
                    iterator_sukcesji++;
                }
                lancuch_binarny = "";
            ...
            System.out.println("KRZYŻOWANIE wybór osobników:");
            double pk = 0.5;
            ...
            for (int i = 0; i<rozmiar_par/2; i++){
                ...
                for (int j = 0; j<zakres_m; j++){
                    ...
                    sukcesja[iterator_sukcesji] += populacja_array_klon[zmienna_pomocnicza_int][j];
                }
                iterator_sukcesji++;
                System.out.println();
                System.out.print(pary[i][1]+". ");
            ...
            double wynik = 0;
            double xprim = 0;
            System.out.println("LĄCZNA ILOSC OSOBNIKÓW DO SUKCESJI");
            int ilosc_osobnikow_sukcesji = 0;
            for (int i = 0; i<sukcesja.length; i++){
                ilosc_osobnikow_sukcesji++;
                if (sukcesja[i] == "")
                    break;
            }

            int[][] sukcesja_tab = new int [ilosc_osobnikow_sukcesji][zakres_m];
            for (int i = 0; i<ilosc_osobnikow_sukcesji-1; i++){
                for (int j = 0; j<zakres_m; j++){
                    zmienna_pomocnicza_int = (int)(sukcesja[i].replace(" ", "").charAt(j));
                    if(zmienna_pomocnicza_int == 48) zmienna_pomocnicza_int = 0;
                    else zmienna_pomocnicza_int = 1;
                    sukcesja_tab[i][j] = zmienna_pomocnicza_int;
                }
                //System.out.println();
            }
        double Fx2[] = new double[ilosc_osobnikow_sukcesji-1];
        double[] ocena_potomkow = new double[n];
        for (int i = 0; i < n; i++) {
            ocena_potomkow[i] = 0;
            Fx2[i] = Fx[i];
        }
        ...
        xprim = 0;
        ///////////////MINIMUM
        for (int i = n; i < ilosc_osobnikow_sukcesji-1; i++) {
            zakres_m_po_odliczeniu = zakres_m;
            zakresik_m = zakres_m - 1;
            ...
            for (int k = ilosc_funkcji - 1; k >= 0; k--) {
                ...
                while (true) {
                    if (zakresik_m == zakres_m_po_odliczeniu || zakresik_m == -1)
                        ...
                        dziesietna += sukcesja_tab[i][zakresik_m] * Math.pow(2, potega);
                        potega++;
        ...
        for (int i = 0; i<ilosc_osobnikow_sukcesji-1; i++){
            System.out.print((i+1)+".\t");
            for (int j = 0; j<zakres_m; j++){
                System.out.print(sukcesja_tab[i][j]+" ");
            }
            System.out.print("\tFx = "+Fx2[i]);
            System.out.println();
        }
        double[] Fx3 = new double [n];
        int[][] sukcesja_koncowa = new int[n][zakres_m];
        Integer[] indexy_sukcesji = new Integer[ilosc_osobnikow_sukcesji];
        for (int i = 0; i<ilosc_osobnikow_sukcesji; i++){
            indexy_sukcesji[i] = i;
        }


        System.out.println("\nMechanizm usuwania osobników: losowo");
        List<Integer> intList2 = Arrays.asList(indexy_sukcesji);
        Collections.shuffle(intList2);
        //zamiana listy spowrotem na tablice Integerow
        intList.toArray(indexy_sukcesji);
        int iterator = 0;
        int iterator2 = 0;
        int[] losowe_indexy_sukcesji = new int[n];
        while (true){
            if (indexy_sukcesji[iterator] != null) {
                //System.out.print(indexy_sukcesji[iterator] + ", ");
                losowe_indexy_sukcesji[iterator2] = indexy_sukcesji[iterator];
                iterator++;
                iterator2++;
            }
            else
        ...
        System.out.println("\tOsobniki, które nie zostały usunięte:");
        System.out.print("\t");
        for (int i = 0; i<n; i++){
            System.out.print(losowe_indexy_sukcesji[i]+" ");
        }
        System.out.println("\nPopulacja losowa:");
        int wartosc = 0;
        for (int i = 0; i<n; i++){
            wartosc = losowe_indexy_sukcesji[i];
            System.out.print(wartosc+".\t");
            for (int j = 0; j<zakres_m; j++){
                System.out.print(sukcesja_tab[losowe_indexy_sukcesji[i]][j]+" ");
            }
            System.out.print("\tFx = "+Fx2[losowe_indexy_sukcesji[i]-1]);
            System.out.println();
        }

        System.out.println("\nMechanizm usuwania osobników: elitarna");
        double[][] elitarna = new double[n][2]; //[index_populacji][wartosc_fx_dla_indexu]
        double[] Fx_min = new double[n];
        double[] Fx_max = new double[n];
        for (int i = 0; i<n; i++){
            elitarna[i][0] = i;
            elitarna[i][1] = Fx2[i];
            Fx_min[i] = Fx2[i];
        ...
        Arrays.sort(Fx_min);
        System.out.println("\tPopulacja po usunięciu najgorzej przystosowanych osobników");
        for (int i = 0; i<ilosc_osobnikow_sukcesji-1; i++){
            System.out.print((i+1)+".\t");
            for (int j = 0; j<zakres_m; j++){
                System.out.print(sukcesja_tab[i][j]+" ");
